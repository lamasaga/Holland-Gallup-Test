...
| 2026-06-24 | v2.0.3 | 修复 34 主题叙事缺失：将 `cliftonstrengths_report_materials.md` 中的完整描述（含标准定义/一句话定义）写入 JSON 并同步到数据库 `theme_descriptions`，恢复报告 Top 5 表格与附录中的主题叙事 | `research/测评研究/data/processed/cliftonstrengths_report_materials.json`, `backend/data/assessment.db`, `backend/app/services/report.py`（验证用）, `scripts/parse_theme_materials.py`, `scripts/sync_theme_descriptions_to_db.py` |
| 2026-06-24 | v2.0.4 | 主题叙事完全双语化：为 34 个主题的 standard_definition、feature、application、blind_spots 补充中文/英文对照；数据模型新增 `_zh` / `_en` 字段；报告 Top 5 表格与附录按字段展示中英双语；新增 `kz-info` 报告样式 | `research/测评研究/data/processed/cliftonstrengths_report_materials.json`, `backend/app/models.py`, `backend/app/import_data.py`, `backend/app/services/report.py`, `backend/data/assessment.db`, `scripts/build_bilingual_template.py`, `scripts/merge_bilingual_translations.py`, `scripts/sync_theme_descriptions_to_db.py` |
| 2026-06-24 | v2.0.5 | 修复 Gallup 180 题中 9 道题的 A/B 场景描述反转问题（Q22、Q23、Q24、Q25、Q26、Q28、Q31、Q32、Q35），并同步更新 SQLite 数据库中对应题的 scenario_hint；脚本自动检查其余题目未发现同类反转 | `research/测评研究/data/processed/gallup_180_questions_list.md`, `backend/data/assessment.db`, `scripts/check_gallup_scenario_reversals.py`, `scripts/fix_gallup_scenario_reversals.py` |
