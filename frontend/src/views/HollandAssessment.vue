<template>
  <div class="container">
    <h1>Holland 兴趣测评</h1>
    <p class="subtitle">根据你对以下活动的喜欢程度，选择 1-5 分</p>
    ...
    <div v-else>
      ...
      <div class="card question-card" v-if="currentQuestion">
        ...
        <p class="statement">{{ currentQuestion.statement_a }}</p>
        <p class="scenario" v-if="currentQuestion.scenario_hint">💡 {{ currentQuestion.scenario_hint }}</p>
        <div class="options">
          <button 
            v-for="score in [1, 2, 3, 4, 5]" 
            :key="score"
            :class="{ selected: answers[currentIndex] !== null && answers[currentIndex] === score }"
...
import { getQuestions, submitHolland } from '../api'
...
const scoreLabels = ['很不喜欢', '不太喜欢', '一般', '比较喜欢', '非常喜欢']
...
onMounted(async () => {
  try {
    questions.value = await getQuestions('holland')
...
function selectAnswer(score) {
  ...
  answers.value = answers.value.map((v, i) => i === currentIndex.value ? score : v)
